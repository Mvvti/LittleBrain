...
class Api:
        ...
        return [
            {
                ...
                "status": n[2],
                "created_at": n[3],
                "remind_at": n[4] if (n[4] is not None and n[5] == 0) else None,
                "pinned": n[6] == 1,
            }
            for n in notes
        ]
    ...
    def update_reminder(self, note_id: int, remind_at):
        from core.db import update_note_reminder

        update_note_reminder(note_id=note_id, remind_at=remind_at, db_path=DB_PATH)
        return {"ok": True}

    def clear_reminder(self, note_id: int):
        from core.db import clear_note_reminder

        clear_note_reminder(note_id=note_id, db_path=DB_PATH)
        return {"ok": True}
    ...
    def toggle_pin(self, note_id: int):
        from core.db import toggle_pin as db_toggle_pin

        db_toggle_pin(note_id=note_id, db_path=DB_PATH)
        return {"ok": True}
